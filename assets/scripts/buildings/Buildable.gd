...
func build(x, y, size_override = size):
	...
	size = size_override
	Grid.set_region(x, y, size.x, size.y, {"id": 29, "building": self}) # Adjust this to be building specific later
...
func update_spawn_tiles():
	...
	for _x in range(location.x-floor(size.x/2)-1, location.x+floor(size.x/2)+2, 1):
		...
		if Grid.in_grid(vector.x, vector.y) and Grid.is_walkable(Grid.get_tile(vector.x, vector.y)):
